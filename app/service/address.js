...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.lbchinfo.lib
    ...
    let [
      ...
      lpp20Service.getAllLPP20Balances(hexAddresses),
      lpp721Service.getAllLPP721Balances(hexAddresses),
  ...
  async getAddressTransactionCount(addressIds, rawAddresses) {
    const {Address: RawAddress, Solidity} = this.app.lbchinfo.lib
  ...
  async getAddressLPP20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.lbchinfo.lib
    const TransferABI = Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressLPP20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.lbchinfo.lib
    const TransferABI = Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressLPP20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN lpp20 ON lpp20.contract_address = log.address
  ...
  async getAddressLPP20TokenMempoolTransactions(rawAddresses, token) {
        ...
        },
        {
          ...
          include: [{
            ...
            include: [{
              ...
              as: 'contract',
              required: true,
              where: {address: token.contractAddress, type: 'lpp20'},
              attributes: []
            }]
          }]
        },
