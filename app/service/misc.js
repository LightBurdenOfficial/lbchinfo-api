...
class MiscService extends Service {
  async classify(id) {
    ...
    const {Block, Transaction, Contract, Lpp20: LPP20, where, fn, literal} = db
    ...
    let lpp20Results = (await LPP20.findAll({
      where: {
        [$or]: [
      ...
      transaction
    })).map(lpp20 => lpp20.contractAddress)
    if (lpp20Results.length === 0) {
      lpp20Results = (await LPP20.findAll({
        where: {
          [$or]: [
    ...
    if (lpp20Results.length) {
      let [{addressHex}] = await db.query(sql`
        SELECT contract.address_string AS address, contract.address AS addressHex FROM (
          SELECT contract_address FROM lpp20_statistics
          WHERE contract_address IN ${lpp20Results}
          ORDER BY transactions DESC LIMIT 1
        ) lpp20_balance
        INNER JOIN contract ON contract.address = lpp20_balance.contract_address
