...
class LPP20Service extends Service {
  async listLPP20Tokens() {
    const db = this.ctx.model
    const {Lpp20Statistics: LPP20Statistics} = db
    const {sql} = this.ctx.helper
    const {gt: $gt} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await LPP20Statistics.count({
    ...
    let list = await db.query(sql`
      SELECT
        ...
        lpp20.name AS name, lpp20.symbol AS symbol, lpp20.decimals AS decimals, lpp20.total_supply AS totalSupply,
        lpp20.version AS version,
        list.holders AS holders,
        list.transactions AS transactions
      FROM (
        SELECT contract_address, holders, transactions FROM lpp20_statistics
        WHERE transactions > 0
        ORDER BY transactions DESC
        LIMIT ${offset}, ${limit}
      ) list
      INNER JOIN lpp20 USING (contract_address)
  ...
  async getAllLPP20Balances(hexAddresses) {
    if (hexAddresses.length === 0) {
      return []
    }
    const {OutputScript, Solidity} = this.app.lbchinfo.lib
    const transferABI = Solidity.lpp20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Lpp20: LPP20, Lpp20Balance: LPP20Balance,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    let list = await LPP20.findAll({
    ...
    let mapping = new Map(list.map(item => [
      item.contract.addressString,
      {
        ...
        symbol: item.symbol,
        decimals: item.decimals,
        balance: item.contract.lpp20Balances.map(({balance}) => balance).reduce((x, y) => x + y),
        unconfirmed: {
          received: 0n,
          sent: 0n
        }
      ...
      if (byteCode.length === 68
        ...
        if (mapping.has(item.output.address.contract.addressString)) {
          ...
          data = {
            ...
            name: item.output.address.contract.lpp20.name,
            symbol: item.output.address.contract.lpp20.symbol,
            decimals: item.output.address.contract.lpp20.decimals,
            balance: 0n,
            unconfirmed: {
              received: 0n,
              sent: 0n
  ...
  async getLPP20Balance(rawAddresses, tokenAddress) {
    const {Address: RawAddress, OutputScript, Solidity} = this.app.lbchinfo.lib
    const transferABI = Solidity.lpp20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Lpp20: LPP20, Lpp20Balance: LPP20Balance,
    ...
    let token = await LPP20.findOne({
      where: {contractAddress: tokenAddress},
      attributes: ['name', 'symbol', 'decimals'],
      transaction: this.ctx.state.transaction
    })
    let list = await LPP20Balance.findAll({
  ...
  async getLPP20BalanceHistory(addresses, tokenAddress) {
    const TransferABI = this.app.lbchinfo.lib.Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    const {
      Header, Transaction,
      EvmReceipt: EVMReceipt, EvmReceiptLog: EVMReceiptLog,
      Contract, Lpp20: LPP20, Lpp20Balance: LPP20Balance,
      literal
    } = db
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM evm_receipt receipt, evm_receipt_log log, lpp20
      WHERE receipt._id = log.receipt_id AND log.address = lpp20.contract_address AND ${{raw: logFilter}}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    if (totalCount === 0) {
      return {totalCount: 0, transactions: []}
    }
    let ids = (await db.query(sql`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM evm_receipt receipt, evm_receipt_log log, lpp20
        WHERE receipt._id = log.receipt_id AND log.address = lpp20.contract_address AND ${{raw: logFilter}}
    ...
    if (list.length > 0) {
      let intialBalanceList = await LPP20Balance.findAll({
        where: {
      ...
      for (let log of logs) {
        ...
        if (item) {
          ...
          result.tokens.push({
            ...
            name: log.lpp20.name.toString(),
            symbol: log.lpp20.symbol.toString(),
            decimals: log.lpp20.decimals,
            amount: delta
          })
        }
      }
  ...
  async getAllLPP20TokenTransactions() {
    const TransferABI = this.app.lbchinfo.lib.Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset, reversed = true} = this.ctx.state.pagination
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM lpp20, evm_receipt_log log
      WHERE lpp20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    let transactions = await db.query(sql`
      SELECT
        ...
        lpp20.name AS name,
        lpp20.symbol AS symbol,
        lpp20.decimals AS decimals,
        evm_receipt_log.topic2 AS topic2,
        evm_receipt_log.topic3 AS topic3,
        evm_receipt_log.data AS data
      FROM (
        SELECT log._id AS _id FROM lpp20, evm_receipt_log log
        WHERE lpp20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
      ...
      INNER JOIN lpp20 ON lpp20.contract_address = evm_receipt_log.address
  ...
  async getLPP20TokenTransactions(contractAddress) {
    const TransferABI = this.app.lbchinfo.lib.Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
  ...
  async getLPP20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Lpp20Balance: LPP20Balance} = db
    const {ne: $ne} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await LPP20Balance.count({
  ...
  async updateLPP20Statistics() {
    const TransferABI = this.app.lbchinfo.lib.Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Lpp20: LPP20, Lpp20Statistics: LPP20Statistics} = db
    const {sql} = this.ctx.helper
    let transaction = await db.transaction()
    try {
      let result = (await LPP20.findAll({attributes: ['contractAddress'], transaction})).map(
        ({contractAddress}) => ({contractAddress, holders: 0, transactions: 0})
      )
      let balanceResults = await db.query(sql`
        SELECT contract_address AS contractAddress, COUNT(*) AS count FROM lpp20_balance
      ...
      await db.query(sql`DELETE FROM lpp20_statistics`, {transaction})
      await LPP20Statistics.bulkCreate(result, {validate: false, transaction, logging: false})
