...
class ContractService extends Service {
  async getContractAddresses(list) {
    const {Address} = this.app.lbchinfo.lib
  ...
  async getContractSummary(contractAddress, addressIds) {
    const {Contract, Lpp20: LPP20, Lpp20Statistics: LPP20Statistics, Lpp721: LPP721} = this.ctx.model
    const {balance: balanceService, lpp20: lpp20Service, lpp721: lpp721Service} = this.ctx.service
    ...
    let [
      ...
      lpp20Service.getAllLPP20Balances([contractAddress]),
      lpp721Service.getAllLPP721Balances([contractAddress]),
    ...
    return {
      ...
      ...contract.type === 'lpp20' ? {
        lpp20: {
          name: contract.lpp20.name,
          symbol: contract.lpp20.symbol,
          decimals: contract.lpp20.decimals,
          totalSupply: contract.lpp20.totalSupply,
          version: contract.lpp20.version,
          holders: contract.lpp20.statistics.holders,
          transactions: contract.lpp20.statistics.transactions
        }
      } : {},
      ...contract.type === 'lpp721' ? {
        lpp721: {
          name: contract.lpp721.name,
          symbol: contract.lpp721.symbol,
          totalSupply: contract.lpp721.totalSupply
        }
      } : {},
    ...
    let [{count}] = await db.query(sql`
      SELECT COUNT(*) AS count FROM (
        ...
        WHERE log.receipt_id = receipt._id
          AND ${this.ctx.service.block.getRawBlockFilter('receipt.block_height')}
          AND contract.address = log.address AND contract.type IN ('lpp20', 'lpp721')
          AND log.topic1 = ${TransferABI.id}
          AND (log.topic2 = ${topic} OR log.topic3 = ${topic})
          AND (
            (contract.type = 'lpp20' AND log.topic3 IS NOT NULL AND log.topic4 IS NULL)
            OR (contract.type = 'lpp721' AND log.topic4 IS NOT NULL)
          )
      ) list
  ...
  async getContractBasicTransactions(contractAddress) {
    const {Address, OutputScript} = this.app.lbchinfo.lib
    const {
