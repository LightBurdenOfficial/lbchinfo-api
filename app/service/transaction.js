...
class TransactionService extends Service {
  async getTransaction(id) {
    const {
      ...
      Contract, Lpp20: LPP20, Lpp721: LPP721,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    const {Address: RawAddress} = this.app.lbchinfo.lib
        ...
        if (output.evmReceipt) {
          ...
          outputObject.evmReceipt.logs = eventLogs.filter(log => log.receiptId === output.evmReceipt._id).map(log => ({
            ...
            ...log.lpp20 ? {
              lpp20: {
                name: log.lpp20.name,
                symbol: log.lpp20.symbol,
                decimals: log.lpp20.decimals
              }
            } : {},
            ...log.lpp721 ? {
              lpp721: {
                name: log.lpp721.name,
                symbol: log.lpp721.symbol
              }
            } : {}
          }))
        }
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.lbchinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.lbchinfo.rpc(this.app.config.lbchinfo.rpc)
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [lpp20TokenTransfers, lpp20TokenUnconfirmedTransfers, lpp721TokenTransfers] = await Promise.all([
      this.transformLPP20Transfers(transaction.outputs),
      confirmations === 0 ? this.transformLPP20UnconfirmedTransfers(transaction.outputs) : undefined,
      this.transformLPP721Transfers(transaction.outputs)
    ])
    ...
    return {
      ...
      lpp20TokenTransfers,
      lpp20TokenUnconfirmedTransfers,
      lpp721TokenTransfers
    }
  }

  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.lbchinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.lbchinfo.lib
  ...
  async transformLPP20Transfers(outputs) {
    const TransferABI = this.app.lbchinfo.lib.Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, data, lpp20} of output.evmReceipt.logs) {
          if (lpp20 && topics.length === 3 && Buffer.compare(topics[0], TransferABI.id) === 0 && data.length === 32) {
  ...
  async transformLPP721Transfers(outputs) {
    const TransferABI = this.app.lbchinfo.lib.Solidity.lpp20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, lpp721} of output.evmReceipt.logs) {
          if (lpp721 && topics.length === 4 && Buffer.compare(topics[0], TransferABI.id) === 0) {
            let [from, to] = await this.ctx.service.contract.transformHexAddresses([topics[1].slice(12), topics[2].slice(12)])
            result.push({
              address: addressHex.toString('hex'),
              addressHex: addressHex.toString('hex'),
              name: lpp721.name,
              symbol: lpp721.symbol,
              ...from && typeof from === 'object' ? {from: from.hex.toString('hex'), fromHex: from.hex.toString('hex')} : {from},
              ...to && typeof to === 'object' ? {to: to.hex.toString('hex'), toHex: to.hex.toString('hex')} : {to},
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.lbchinfo.lib
    const {
