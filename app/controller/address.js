...
class AddressController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      lpp20Balances: summary.lpp20Balances.map(item => ({
      ...
      lpp721Balances: summary.lpp721Balances.map(item => ({
        address: item.addressHex.toString('hex'),
        addressHex: item.addressHex.toString('hex'),
        name: item.name,
        symbol: item.symbol,
  ...
  async lpp20TokenBalance() {
    let {ctx} = this
    let {address, token} = ctx.state
    if (token.type !== 'lpp20') {
      ctx.body = {}
    }
    let {name, symbol, decimals, balance, unconfirmed} = await ctx.service.lpp20.getLPP20Balance(address.rawAddresses, token.contractAddress)
    ctx.body = {
      name,
      symbol,
      decimals,
  ...
  async lpp20TokenTransactions() {
    let {ctx} = this
    let {address, token} = ctx.state
    let {totalCount, transactions} = await ctx.service.address.getAddressLPP20TokenTransactions(address.rawAddresses, token)
    ctx.body = {
      totalCount,
      transactions: transactions.map(transaction => ({
        transactionId: transaction.transactionId.toString('hex'),
  ...
  async lpp20TokenMempoolTransactions() {
    let {ctx} = this
    let {address, token} = ctx.state
    let transactions = await ctx.service.address.getAddressLPP20TokenMempoolTransactions(address.rawAddresses, token)
  ...
  async lpp20BalanceHistory() {
    const {Address} = this.app.lbchinfo.lib
    let {ctx} = this
    let tokenAddress = null
    if (ctx.state.token) {
      if (ctx.state.token.type === 'lpp20') {
    ...
    let {totalCount, transactions} = await ctx.service.lpp20.getLPP20BalanceHistory(hexAddresses, tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
