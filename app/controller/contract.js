...
class ContractController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      ...summary.type === 'lpp20' ? {
        lpp20: {
          name: summary.lpp20.name,
          symbol: summary.lpp20.symbol,
          decimals: summary.lpp20.decimals,
          totalSupply: summary.lpp20.totalSupply.toString(),
          version: summary.lpp20.version,
          holders: summary.lpp20.holders,
          transactions: summary.lpp20.transactions
        }
      } : {},
      ...summary.type === 'lpp721' ? {
        lpp721: {
          name: summary.lpp721.name,
          symbol: summary.lpp721.symbol,
          totalSupply: summary.lpp721.totalSupply.toString()
        }
      } : {},
  ...
  async lpp20BalanceHistory() {
    ...
    let {totalCount, transactions} = await ctx.service.lpp20.getLPP20BalanceHistory([ctx.state.contract.contractAddress], tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
